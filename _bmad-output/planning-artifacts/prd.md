...
classification:
  projectType: 'web_app'
  projectTypeDetail: 'Web application (6-product platform: KnowledgeEnroll, KnowledgeLecture, KnowledgeCollege, KnowledgeGraduate, KnowledgeGateway, KnowledgeOps)'
  domain: 'knowledge_management'
  domainDetail: 'Knowledge management / content intelligence'
  complexity: 'medium-high'
  complexityNotes: 'Technically ambitious (multi-host, 11-state machine, MCP Gateway transcript pipeline) but no regulatory burden'
  projectContext: 'greenfield'
  projectContextDetail: 'Greenfield with validated foundations (spike proved SurrealDB + MCP Gateway + n8n pipeline)'
inputDocuments:
...
> [Updated 2026-03-31: spike learnings — reflects actual deployed state]

**KnowledgeStack** is not a single application — it is a platform composed of 6 distinct products that work together to transform YouTube content from trusted experts into a searchable, AI-queryable knowledge repository. Each product owns a clear stage of the pipeline: content enters through **KnowledgeEnroll**, is listened to and watched in **KnowledgeLecture**, gets enriched by **KnowledgeCollege**, is refined into golden standards by **KnowledgeGraduate**, is exposed to external tools via **KnowledgeGateway**, and the whole system is managed through **KnowledgeOps**.

It solves a core problem: you can't scan a 3-hour video to find the 5-10 minutes worth watching. KnowledgeStack ingests, transcribes, enriches, and structures content from 50 curated channels across 7 domains (AI/Tech, Business, Political, Mindset, Health, General, Faith) — making expert knowledge instantly accessible at the segment level.

**Current State (2026-03-31):** The spike/pre-alpha phase validated the core pipeline end-to-end. 50 channels are monitored, 1,017 videos indexed in SurrealDB with 83,528 segments, n8n orchestrator running every 5 minutes with 95% success rate. The pipeline is live at knowledge.nextlevelfoundry.com with an admin UI, Speakr for transcript browsing, and a landing page. Key architectural pivots from the spike: SurrealDB replaced Qdrant (unified vector+graph+document store), MCP Gateway replaced youtube-transcript-api for transcript fetching, and Speakr's role shifted from authoritative data store to UI/listening layer with data flowing in parallel to both Lecture (Speakr) and College (SurrealDB).

### The 6 Products

| Product | Tier | What It Does | Tech Foundation |
|---------|------|-------------|-----------------|
| **KnowledgeEnroll** | 1 — Ingestion | Monitors RSS feeds, fetches transcripts via MCP Gateway, deduplicates, captures metadata, manages channel subscriptions | n8n workflows, MCP Gateway (Helicarrier), PostgreSQL |
| **KnowledgeLecture** | 2 — Lecture Hall | Listen to and watch transcribed expert lectures with full-text search, per-recording AI chat, tagging, bookmarks, multi-user access | Speakr (adopted open-source, unmodified) |
| **KnowledgeCollege** | 3 — Intelligence | Embeds transcripts into vector store, enables semantic search, enriches with entities/topics/speakers | SurrealDB (vector+graph+document), LiteLLM, kg-gen + spaCy |
| **KnowledgeGraduate** | 4 — Refinement | Curates "golden standard" content, refines knowledge through human review, creates premium excerpts | Admin UI, curation workflows |
| **KnowledgeGateway** | 5 — Access | Exposes the knowledge repository to external apps and AI tools via REST API and MCP server | Python/Flask, MCP Server |
| **KnowledgeOps** | Cross-cutting | Pipeline monitoring, admin UI, status digests, failed item management, admin tooling, DevOps lifecycle | n8n, Admin API (Banner:5020), Grafana, Loki |
...
> [Updated 2026-03-31: spike learnings — reflects actual deployed topology]

Multi-host deployment on internal network (10.0.0.x):
- **Banner** (10.0.0.33): Speakr (5000), Admin API (5020), Embedding service (5030), Landing page (5001), PostgreSQL (5019), SurrealDB (5040)
- **Helicarrier** (10.0.0.27): n8n workflow engine, MCP Gateway (2780), LiteLLM proxy (2764), Traefik reverse proxy
- **Coulson**: Grafana + Loki + Prometheus (monitoring)
- **Fury/Synology NAS**: Audio file storage
- **Jarvis**: NOT currently used (MCP Gateway on Helicarrier handles transcripts; WhisperX not needed for YouTube captions)
- **Domain**: knowledge.nextlevelfoundry.com (Traefik routes: /enroll/, /lecture/, /ops/, landing page at root)
...
**Downstream App Success (KnowledgeGateway -- Future):**
...
> [Updated 2026-03-31: spike outcomes — most 3-month targets achieved or exceeded during spike]

- ~~Pipeline reliably ingests new content from ~50 channels with <5% failure rate~~ **ACHIEVED:** 50 channels monitored, 95% pipeline success rate, queue cleared
- ~~Speakr instance contains 500+ transcribed recordings~~ **EXCEEDED:** 1,017 videos indexed in SurrealDB with 83,528 segments
- ~~Search returns usable results for queries across all 5 domains~~ **PARTIALLY:** Keyword search works across 7 domains; semantic search blocked by zero embeddings (83K segments need embedding generation)
- ~~Canary channel testing~~ **SUPERSEDED:** Pipeline runs every 5 min; reliability proven by throughput, not canary testing
- ~~Operational visibility~~ **PARTIALLY:** Admin UI at knowledge.nextlevelfoundry.com/enroll/ provides dashboard, channels, pipeline, videos, tags. Slack alerts NOT yet integrated.
- ~~Channel monitor page~~ **ACHIEVED:** Admin UI shows all channels with status

**Remaining for full MVP-1 validation:**
- Generate embeddings for 83,528 segments (~$2 via text-embedding-3-small)
- Persist tag schema in init.surql (currently lost on SurrealDB restart)
- Automate enrichment in pipeline (kg-gen currently manual)
- Integrate Slack alerting for pipeline failures
- Authenticate with Authentik (currently everything is open)
...
- KnowledgeGateway API serving at least 1 downstream application (e.g., LobeChat)
...
> [Updated 2026-03-31: spike learnings — adjusted for actual architecture]

| Metric | Target | Why It Matters | Spike Status |
|--------|--------|----------------|-------------|
| Ingestion pipeline uptime | >95% (auto-recovery) | RSS -> n8n -> SurrealDB + Speakr must run unattended | **95% achieved** |
| Transcript accuracy | "Good enough" (MCP Gateway fetches YouTube captions) | Dependency -- we don't re-solve transcription | **Working** |
| Search relevance | Aspiration: relevant results in top 3. Blocked on embeddings. | Core value prop long-term, but not an MVP hard gate | **Blocked** (zero embeddings) |
| Dedup accuracy | <1% false duplicates | PostgreSQL ON CONFLICT + SurrealDB UUIDv5 | **Working** |
| Pipeline failure rate | <5% per batch run | Retries + dead-letter queue handle transient failures | **5% achieved** |
| System recovery | <30 min from any single-service failure | Docker Compose restart + health checks | **Untested** |
| Speakr API response | Monitor, don't control. Flag if degradation observed | Dependency we track, not engineer | **Working** |
...
| Product | Gate | Metric | Spike Status |
|---------|------|--------|-------------|
| KnowledgeEnroll | Automated ingestion running | New videos from 50 channels appearing within 24h of publish | **ACHIEVED** — 50 channels, 95% success, 5-min cycle |
| KnowledgeLecture | Searchable repository | Speakr running with 500+ recordings, search works, per-recording chat works | **ACHIEVED** — Speakr live at /lecture/, 1,017 videos |
| KnowledgeCollege | Embeddings + semantic search | 80%+ segments embedded, semantic search returning relevant results | **MVP-2** — 83K segments indexed but zero embeddings |
| KnowledgeGraduate | Refinement active | Curation workflows after repository has enriched content | **Growth** — deferred |
| KnowledgeGateway | API accessible | REST API + API keys serving at least 1 downstream consumer | **MVP-2 or Growth** — data exists, API layer is thin |
| Operations | Visibility | Pipeline health surfaced via admin UI + Slack alerts + n8n logs | **PARTIAL** — Admin UI deployed, Slack alerts pending |
...
> [Updated 2026-03-31: spike learnings — Jarvis/WhisperX removed, MCP Gateway added]

- Speakr PostgreSQL on Banner (port 5019) — **deployed**
- SurrealDB on Banner (port 5040) — **deployed**
- MCP Gateway on Helicarrier (port 2780) — **deployed** (undocumented dependency on Helicarrier)
- LiteLLM proxy on Helicarrier (port 2764) — **deployed** (embedding generation)
- n8n workflows on Helicarrier — **deployed** (3 workflows running)
- Traefik on Helicarrier — **deployed** (routes knowledge.nextlevelfoundry.com)
- NAS mount for Speakr audio storage (Fury/Synology)
- ~~WhisperX on Jarvis~~ — NOT NEEDED: MCP Gateway fetches YouTube captions directly, no audio transcription required
...
14. **KnowledgeGateway API**: REST endpoints exposing structured knowledge to external apps
...
### Product Architecture (6-Product Platform)
...
| **KnowledgeCollege** | 3 | Intelligence layer (SurrealDB vector+graph, enrichment, semantic search, entity graphs) |
| **KnowledgeGraduate** | 4 | Refinement layer (golden standards, human curation, premium excerpts) |
| **KnowledgeGateway** | 5 | API + integrations (REST, MCP, LobeChat, downstream apps) |
...
| 5 | Curated Export for AI | Matt (Downstream) | MVP/Growth | KnowledgeLecture/KnowledgeGateway |
| 6 | Curator Graduation | Sarah -> Curator | Growth | KnowledgeEnroll |
| 7 | AI Tool Integration (MCP + REST) | Claude/LobeChat/OpenClaw | Vision | KnowledgeGateway |
...
**Opening Scene:** Matt has just deployed KnowledgeStack. Speakr is running on Banner, n8n is on Helicarrier, MCP Gateway handles transcripts. The system is empty. He opens the channel management interface with his list of ~50 channels organized by tier.
...
**Persona:** Matt (Downstream Consumer) | **Product:** KnowledgeLecture/KnowledgeGateway
...
**Rising Action (Growth -- API):** Matt uses the KnowledgeGateway API to query structured transcript chunks with metadata. Pipes this directly into OpenClaw.
...
**Reveals requirements for:** Search with channel filtering (Speakr -- verify), transcript export/download (Speakr -- verify), KnowledgeGateway REST API (Growth), MCP server (Vision), metadata in export (source, timestamp, speaker).
...
**Persona:** Claude Code / LobeChat / OpenClaw (via MCP or REST API) | **Product:** KnowledgeGateway
...
**Reveals requirements for:** MCP server implementation (tool definitions for search, retrieve, list channels), REST API endpoints (KnowledgeGateway), structured response format (transcript + metadata + citations), authentication for API consumers, rate limiting.
...
| KnowledgeGateway REST API | J5, J7 | Growth/Vision | No -- KnowledgeGateway |
| Curator role + self-service channel add | J6 | Growth | No -- KnowledgeEnroll |
| MCP server for AI tool integration | J7 | Vision | No -- KnowledgeGateway |
...
See **Non-Functional Requirements > Performance** (NFR-P1 through NFR-P9) for all measurable performance targets across KnowledgeEnroll, KnowledgeLecture, KnowledgeOps, and KnowledgeGateway.
...
- Custom web interfaces (KnowledgeEnroll, KnowledgeCollege, KnowledgeGraduate, KnowledgeGateway, KnowledgeOps): WCAG 2.1 AA compliance via Sunflower UI (Growth phase). See NFR-A1 through NFR-A4.
...
- All custom web interfaces for Tiers 1, 3, 4, 5, and KnowledgeOps use Sunflower UI (NLF custom UI kit) — NOT Speakr's Vue.js 3 stack
...
> [Updated 2026-03-31: spike learnings — pipeline is running, focus shifts to hardening and gaps]

**Gate:** Pipeline autonomous, Speakr running, admin UI operational, Slack alerting functional, auth in place

**Status:** Substantially achieved during spike. Remaining work is hardening, not greenfield.

| Product | What Ships | Build vs Adopt | Spike Status |
|---------|-----------|---------------|-------------|
| **KnowledgeEnroll** | Channel mgmt, RSS monitoring, dedup, retry/DLQ, MCP Gateway transcript fetch | Build (n8n workflows) | **Running** — 3 n8n workflows live |
| **KnowledgeLecture** | Search, chat, tags, transcripts, multi-user, SSO | Adopt (Speakr) | **Running** — Speakr at /lecture/ |
| **KnowledgeOps** | Admin UI, Slack alerts, status digests, stale-channel alerts, DLQ mgmt | Build (Admin API + n8n + Slack) | **Partial** — Admin UI deployed, Slack pending |

**Remaining MVP-1 gaps (from spike):**
- Slack alerting integration (pipeline failures, stale channels)
- Authentik SSO integration (currently no auth)
- Tag schema persistence in init.surql
- Automated enrichment in pipeline (kg-gen currently manual)

**Core Journeys:** J1, J1B, J2, J3, J3B, J4, J5 (manual), J8, J9, J10

### MVP-2: Intelligence + Search + Access

> [Updated 2026-03-31: spike learnings — College promoted from Growth to MVP-2; semantic search IS the product]

**Gate:** 80%+ segments have embeddings, semantic search demonstrably better than keyword alone; basic API access for at least one downstream consumer

**Why College moved to MVP-2:** The spike proved keyword search alone doesn't solve the core use case ("find that segment where Huberman explained X"). The data is in SurrealDB with 83K segments ready for embedding. The embedding cost is ~$2. Semantic search is the product differentiator, not a growth feature.

| Product | What Ships | Build vs Adopt | Spike Status |
|---------|-----------|---------------|-------------|
| **KnowledgeCollege** | Embedding generation, semantic search, basic entity extraction, tag quality improvement | Build (SurrealDB + LiteLLM) | **Data ready** — 83K segments, zero embeddings |
| **KnowledgeGateway** (basic) | REST API exposing search + transcript retrieval + API keys | Build (Python/Flask) | **Not started** — but data layer exists |

**Key deliverables:**
- Generate embeddings for 83,528 segments (~$2 via text-embedding-3-small)
- Fix tag quality (95% classified as "concept" — products, companies, people not typed correctly)
- Basic Gateway API with API key authentication
- Cost: ~$2 for full corpus embedding, ~$15-30 for entity enrichment

**Enables:** J2 dead-end recovery solved (synonym/semantic matching), J5 full (API-based export), J7 partial (REST access)

### Growth (Post-MVP-2)

> [Updated 2026-03-31: spike learnings — KnowledgeGraduate + advanced enrichment + Grafana]

Growth specifics will sharpen as MVP-1 and MVP-2 teach us what matters. Current candidates:

- **KnowledgeGraduate** — curation workflows, golden standard content, premium excerpts
- Advanced entity enrichment (expert profiles, authority scoring, cross-channel connections)
...
- Slack integration for alerts and digests (if not completed in MVP-1)

### Vision

> [Updated 2026-03-31: spike learnings — Gateway may move earlier if testers need API access]

- **KnowledgeGateway** full (MCP server + REST API) — upgrades from basic MVP-2 API to MCP+REST (J7 full)
...
- Downstream integrations (Claude Code, OpenClaw, custom apps)

### Dependency Update Management (KnowledgeOps)

**Pattern:** Automated release monitoring with LLM-assisted analysis

> [Updated 2026-03-31: spike learnings — LiteLLM proxy replaces Jarvis for LLM analysis]

| Dependency | Monitor Method | Analysis |
|-----------|---------------|----------|
| Speakr | GitHub releases (RSS/API) | LLM summarizes impact on our usage (API, search, chat, SSO, export) |
| n8n | GitHub releases | LLM flags breaking workflow changes |
| SurrealDB | GitHub releases | LLM flags API changes |

**Workflow:** Release detected -> fetch notes -> LLM summary (via LiteLLM proxy) -> Slack post -> Matt decides (update/skip/investigate)

**Scope:** MVP-1 for Speakr + n8n + SurrealDB. Expand as products come online.

### Risk Mitigation Strategy

> [Updated 2026-03-31: spike learnings — Jarvis/WhisperX risk removed, new risks added]
...
| MCP Gateway availability (Helicarrier) | Technical | Medium | Single point of failure for transcript fetching; pipeline queues items if down |
...
| SurrealDB data persistence | Technical | Medium | Tag schema not in init.surql — restart loses tags. Fix in MVP-1 |
| Embedding cost at scale | Financial | Low | ~$0.00002/segment; full corpus ~$2; ongoing ~$2-5/month |
| Tag quality (95% "concept") | Data Quality | Medium | kg-gen + spaCy entity typing needs tuning; products/companies/people not classified |
| ~~WhisperX GPU availability (Jarvis)~~ | ~~Technical~~ | ~~Resolved~~ | MCP Gateway eliminated Jarvis dependency for YouTube content |
...
- **FR29** [MVP-1]: System provides speaker diarization for multi-speaker content (Speakr/WhisperX if audio-based; YouTube captions via MCP Gateway for caption-based content)

### Content Intelligence (KnowledgeCollege)

- **FR30** [MVP-2]: System embeds transcript content into SurrealDB vector store for semantic search
...
### Content Distribution (KnowledgeGateway)
...
- **FR52** [MVP-1]: System monitors dependency releases (Speakr, n8n, SurrealDB) via GitHub RSS/API
- **FR53** [MVP-1]: System generates LLM-assisted impact summaries for dependency updates (via LiteLLM proxy)
...
- **FR61** [MVP-2]: Admin can create API keys for external service access to KnowledgeGateway
...
- **FR71** [MVP-1]: System performs periodic Speakr health checks (API responsiveness, MCP Gateway availability)
...
- **FR74** [MVP-2]: KnowledgeGateway REST API provides interactive Swagger/OpenAPI documentation page with all endpoints, parameters, and response schemas
...
- **FR93** [MVP-2]: System includes SurrealDB vector+graph store in backup/restore procedures
...
- **FR102** [MVP-1]: System performs AI-assisted pre-ingestion analysis of video descriptions to extract candidate guest speakers and key topics using LLM (via LiteLLM proxy)
...
### KnowledgeGateway API Rate Limiting & Monitoring

- **FR105** [MVP-2]: KnowledgeGateway API enforces rate limiting per API key (configurable thresholds)
- **FR106** [MVP-2]: System monitors and logs all API calls to KnowledgeGateway (caller, endpoint, response time, status code)
...
- **FR120** [MVP-2]: System automatically embeds new recordings into SurrealDB when ingested into KnowledgeLecture (event-driven or scheduled sync)
...
### KnowledgeGateway API Completeness

- **FR123** [MVP-2]: KnowledgeGateway API supports filtered queries (by channel, date range, domain, entity, topic)
- **FR124** [MVP-2]: KnowledgeGateway API supports paginated responses with configurable page size
- **FR125** [MVP-2]: KnowledgeGateway API returns standardized error responses with error codes and descriptive messages
- **FR126** [MVP-2]: KnowledgeGateway API supports versioning to maintain backward compatibility across releases
...
- **FR145** [Vision]: KnowledgeGateway API exposes topic coverage analysis endpoints enabling downstream tools to identify knowledge blind spots and recommend underexplored domains
...
- **FR147** [MVP-2]: KnowledgeGateway API requires end-user identification parameter for analytics-eligible endpoints and supports it as optional for basic query endpoints
...
- **FR156** [MVP-1]: System tracks installed versions of all dependencies (Speakr, n8n, SurrealDB, LiteLLM, MCP Gateway, kg-gen) and displays current versions on an ops dashboard
...
| NFR-P3 | ~~WhisperX transcription speed (Jarvis)~~ | ~~<0.5x real-time~~ | ~~MVP-1~~ | **Removed:** MCP Gateway fetches YouTube captions directly; no audio transcription needed |
...
| NFR-P8 | KnowledgeGateway API response time (p95) | <500ms for search, <200ms for retrieval | MVP-2 | API consumer experience |
...
| NFR-S3 | API authentication | All KnowledgeGateway API calls require valid API key | MVP-2 | Prevent unauthorized access |
...
| NFR-SC5 | Pipeline parallelism | Process multiple channels concurrently without resource contention on MCP Gateway and LiteLLM proxy — batch size and rate limiting tuned | MVP-1 | Transcript fetch + embedding throughput management |
| NFR-SC6 | ~~GPU utilization monitoring~~ | ~~Jarvis GPU tracking~~ | ~~MVP-2~~ | **Removed:** No GPU dependency; monitor MCP Gateway + LiteLLM throughput instead |
...
| NFR-R7 | Graceful degradation | If MCP Gateway or LiteLLM is down, pipeline queues items and resumes when available — no data loss | MVP-1 | Transcript fetch is single point of failure |
...
> **Status:** Spike complete (2026-03-22). SurrealDB validated as database choice.

### Database Architecture Decision

**Decision:** SurrealDB selected for KnowledgeCollege based on successful spike validation. Replaces original Qdrant plan.

| Aspect | Original Plan (Qdrant) | Selected (SurrealDB) |
|--------|----------------------|--------------------------------|
...
- Hybrid queries: Working (keyword + vector + filters in single query)
- Scale tested: 16,607 segments, 205 videos
- RAG pipeline: Functional with keyword and semantic search modes
- Surrealist GUI: Accessible via Cloudflare tunnel

**Spike Artifacts:**
- Schema: `spike/surreal-rag/schema/init.surql`
- Loader: `spike/surreal-rag/scripts/load_to_surrealdb.py`
- RAG Query: `spike/surreal-rag/scripts/ask_transcripts.py`
- Docker: `spike/surreal-rag/docker-compose.yml`

**Remaining for Production:**
- File-based persistence (memory mode used in spike)
- Backup/restore procedures
- Production hardening

**PRD Archival Note:**
- Original plan (Qdrant): Archived as `PRD_Original.md`
- This document updated to reflect SurrealDB decision (2026-03-22)
...
> [Added 2026-03-31: spike learnings — based on actual pipeline costs]

### AI Processing Costs

| Operation | Cost per Unit | Full Corpus | Ongoing (est.) |
|-----------|--------------|-------------|----------------|
| Embedding generation | ~$0.00002/segment (text-embedding-3-small) | ~$2 (83K segments) | ~$2-5/month (50 new videos/week x 50 segments) |
| Entity enrichment (kg-gen) | ~$0.01-0.03/video (gpt-4o-mini) | ~$15-30 (1K videos) | ~$2-5/month |
| Pre-ingestion analysis | ~$0.001/video (gpt-4o-mini) | Negligible | <$1/month |
| LLM dependency analysis | ~$0.01/release | Negligible | <$1/month |

### Infrastructure Costs

All self-hosted on existing infrastructure. No incremental cloud costs beyond:
- SurrealDB storage on Banner (currently ~2GB for 83K segments)
- PostgreSQL storage on Banner (pipeline state, ~50MB)
- NAS storage for audio files (if using Speakr with audio)

### Total Estimated Monthly Cost

- **Steady state:** ~$5-10/month in AI API costs (embeddings + enrichment)
- **One-time backfill:** ~$17-32 for full corpus embedding + enrichment

## Spike Learnings (2026-03-31)

> [Added 2026-03-31: comprehensive record of what the spike phase proved and changed]

### What Was Validated

| Area | Finding | Confidence |
|------|---------|------------|
| SurrealDB as unified store | Vector + graph + document in one DB works. HNSW index, graph traversal <1ms, hybrid queries functional | High |
| MCP Gateway for transcripts | Fetches YouTube captions reliably without audio download or WhisperX | High |
| n8n pipeline orchestration | 3 workflows running autonomously: RSS Monitor, Video Ingest Orchestrator, Embedding Sync | High |
| Scale handling | 1,017 videos, 83,528 segments, 50 channels — no performance issues | High |
| Parallel data flow | Enroll feeds both Lecture (Speakr) and College (SurrealDB) in parallel, not sequentially | High |
| Entity extraction | kg-gen + spaCy-entity-linker produces Wikidata-grounded tag hierarchies (376 tags) | Medium (quality issues) |

### Architecture Pivots

| Original Plan | Actual | Why |
|--------------|--------|-----|
| Qdrant for vectors | SurrealDB | Unified vector+graph+document eliminates 2-database complexity |
| youtube-transcript-api + yt-dlp | MCP Gateway (Helicarrier:2780) | Single endpoint for transcripts; eliminates Jarvis/WhisperX dependency |
| Speakr as authoritative data store | Speakr as UI/listening layer | Data flows RSS -> MCP Gateway -> SurrealDB directly; Speakr receives in parallel |
| Sequential: Enroll -> Lecture -> College | Parallel: Enroll -> (Lecture + College) | Pipeline proved both stores can be fed independently |
| Jarvis for GPU transcription | Not needed | YouTube captions sufficient; MCP Gateway handles fetching |
| KnowledgeCollege in "Growth" phase | KnowledgeCollege in MVP-2 | Semantic search IS the product; keyword search alone doesn't solve the core use case |
| Domain: knowledge.nextlevelguild.com | knowledge.nextlevelfoundry.com | Domain changed during spike |

### What's Not Working

| Issue | Impact | Fix Effort |
|-------|--------|-----------|
| Zero embeddings (83K segments) | Semantic search completely blocked | Low (~$2, automated script) |
| Tag quality (95% "concept") | Products, companies, people not typed correctly | Medium (tune kg-gen/spaCy) |
| Enrichment not automated | kg-gen runs manually, not in pipeline | Medium (n8n workflow addition) |
| No Slack alerts | Ops monitoring is UI-only | Medium (n8n workflow) |
| No auth | Everything is open, no Authentik integration | Medium |
| Tag schema not in init.surql | SurrealDB restart loses tag tables | Low (schema update) |

### Actual n8n Workflows (Deployed)

| Workflow | ID | Trigger | Status |
|----------|----|---------|--------|
| RSS Channel Monitor | jSIl3ztDpfyEqDzN | Schedule (5 min) | Running |
| Video Ingest Orchestrator | wDHhaqklto0ENwSK | Claimed from PostgreSQL | Running |
| Embedding Sync | (TBD) | Schedule (5 min) | Running |
...
- [ADR-001](../../docs/architecture/adrs/ADR-001-surrealdb-unified-store.md) - SurrealDB decision record
