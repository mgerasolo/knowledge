...
- **Status codes:** `200` ingested (or already held) · `400` bad input (unparseable reference, invalid tags or domain) · `404` the video itself is unreadable (bad ID, private, deleted) · `409` a stream that hasn't finished — captions don't exist yet, retry after it ends · `422` readable video with no captions · `429` YouTube is rate-limiting caption requests, retry later · `503` the metadata lookup failed on our side (transient — carries `retryable: true`) · `502/504` internal pipeline unreachable/timed out.
- On an already-held video the response also reports honestly whether the video is actually in the search index: `indexed: false` with an `index_error` means the transcript file exists but search will not find it until it is replayed.
